...
'''
알고리즘:
	1. 좀비의 위치는 바뀌지만 인덱스는 바뀌지 않는다.
	2. 떨어지는 시간과 방향만 놓고 봤을 때 각각의 시간과 방향은 충돌을 고려했을 때와 고려하지 않았을 때 같다.
	(길이가 10인 길에 (5, 4), (7, -5)가 있다고 가정했을 때
	충돌을 고려하지 않은 경우 id 4인 좀비는 5초 뒤에 오른쪽으로 떨어지고, -5인 좀비는 7초 뒤에 왼쪽으로 떨어진다.
	충돌을 고려하는 경우 4인 좀비는 7초 뒤에 왼쪽으로 떨어지고 -5인 좀비는 5초 뒤에 오른쪽으로 떨어진다.
	5초 뒤에 오른쪽으로, 7초 뒤에 왼쪽으로 떨어지는 사실은 변하지 않는다.)
	즉, 충돌을 무시하더라도 충돌을 고려했을 때와 같은 시간에 같은 위치로 좀비가 떨어진다.
	그러나 떨어지는 id 순서는 달라지므로, 어느 좀비가 먼저 떨어지는지를 기억해야 한다.
	이를 위해 덱에 입력받은 순서대로 좀비의 id를 저장하고, 양 끝에 있는 좀비를 확인하여 떨어지는지 아닌지 확인한다.
	그리고 음수 id를 가진 좀비는 [위치, id]로, 양수 id를 가진 좀비는 [L-위치, id]로 저장한 배열을 만들어 정렬한다.
	(음수 id는 왼쪽으로 가므로 입력받은 위치 그대로 저장, 양수 id는 오른쪽으로 가므로 길의 길이 - 위치로 저장)
	id를 저장한 덱과 정렬된 배열을 바탕으로 연산을 진행한다.
	
	만약 좀비가 동시에 떨어진다면(정렬된 배열 첫 번째, 두 번째 요소의 0번째 인덱스 값이 같은 경우 이동 거리가 같으므로 동시에 떨어진다)
	덱의 양 끝의 id를 비교해서 더 작은 값을 가진 좀비부터 떨어트린다(pop한 후 ans에 append)
	이 때, 두 마리의 좀비를 떨어트렸으므로 i는 2만큼 증가시킨다.
	동시에 떨어지지 않는다면 정렬된 배열의 첫 번째 요소의 1번 인덱스 값을 확인하여
	음수인 경우 왼쪽 좀비를 떨어트리고 양수인 경우 오른쪽 좀비를 떨어트린다.
	(떨어지는 방향 또한 충돌을 고려하지 않았을 때와 똑같기 때문에)
	이 과정을 덱이 빌 때 까지 반복한 후 ans[k-1]을 출력한다.

수행시간:
	반복문을 한 번만 사용하나, sort()를 이용하므로 O(nlogn)이다.
'''